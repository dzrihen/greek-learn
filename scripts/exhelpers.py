# -*- coding: utf-8 -*-
"""Exercise builders for Greek curriculum generation."""
import random

def words_of(ru: str):
    return [w for w in ru.replace("?", "").replace("!", "").replace(".", "").replace(",", "").replace("·", " ").replace(";", " ").split() if w]
...
WRONG_POOL = [
    ...
    "אנחנו לומדים יוונית", "היא קוראת ספר", "הם עובדים יחד", "זה קרוב מכאן",
    "אני צריך עזרה", "בואו נשב כאן", "החנות סגורה", "מחר יש מבחן",
]

DISTRACTORS_RU = [
    "και", "αλλά", "ήδη", "πολύ", "ακόμα", "μόνο", "εδώ", "εκεί",
    "τώρα", "μετά", "πάντα", "ποτέ", "ίσως", "πρέπει", "μπορώ",
    "σπίτι", "πόλη", "δουλειά", "ώρα", "μέρα", "πρωί", "βράδυ",
]
